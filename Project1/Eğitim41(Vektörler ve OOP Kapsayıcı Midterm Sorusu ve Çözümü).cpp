...
public:
	...
	string getIsým() const { return isim; }
	string getKadro() const { return kadro; }

	void print() const {
		cout << "Personel No: " << personelNo << " | Isim: " << isim << "| Kadro: " << endl;
	}
};
...
void personelEkle(vector<Personel>& vPersoneller) {
	...
	//vPersoneller.push_back(no, isim, kadro);
...
void personelAra(const vector<Personel>& vPersoneller) {
	int no;
	cout << "Aramal istediginiz personel numarasini giriniz = ";
	cin >> no;
